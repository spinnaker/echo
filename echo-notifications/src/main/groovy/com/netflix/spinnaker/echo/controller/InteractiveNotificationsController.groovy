...
@RequestMapping("/interactive-notifications")
@RestController
@Slf4j
class InteractiveNotificationsController {
  private Collection<InteractiveNotificationService> notificationServices
  private InteractiveNotificationCallbackHandler callbackHandler

  @Autowired
  InteractiveNotificationController(
    Collection<InteractiveNotificationService> notificationServices,
  ...
  /**
   * Provides an endpoint for other Spinnaker services to send out interactive notifications to users.
   * Processing of the request is delegated to an implementation of {@link InteractiveNotificationService}

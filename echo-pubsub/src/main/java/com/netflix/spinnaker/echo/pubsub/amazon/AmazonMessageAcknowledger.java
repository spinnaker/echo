...
public class AmazonMessageAcknowledger implements MessageAcknowledger {
  ...
  public void ack() {
    ...
    try {
      ...
      log.warn("Error deleting message, reason: {} (receiptHandle: {})", e.getMessage(), message.getReceiptHandle());
    }
  }

  @Override
  public void nack() {
    // set visibility timeout to 0, so that the message can be processed by another queue
    try {
      amazonSQS.changeMessageVisibility(queueUrl, message.getReceiptHandle(), 0);
      log.debug("changed visibility timeout of message from " + queueUrl);
    } catch (ReceiptHandleIsInvalidException e) {
      log.warn("Error nack-ing message, reason: {} (receiptHandle: {})", e.getMessage(), message.getReceiptHandle());
    }
  }
